...
const WORK_FILE = "ethereum-utils.transfer-ether.work.json"
...
const doExample = () => {
  const config = CommonUtils.readFileAsJsonOrFatal(path.join(__dirname, "../../config-examples/transfer-ether.json"))
  console.log("\nConfig example:\n")
  console.log(config)
}
...
export const TransferEtherModule: CommandModule = {
  command: "transfer-ether  <command>",
  builder: {
    config: {
      type: "string"
    }
