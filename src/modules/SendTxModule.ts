...
const WORK_FILE = "ethereum-utils.send-tx.work.json"
...
const doExample = () => {
  const config = CommonUtils.readFileAsJsonOrFatal(path.join(__dirname, "../../config-examples/send-tx.json"))
  console.log("\nConfig example:\n")
  console.log(config)
}
...
export const SendTxModule: CommandModule = {
  command: "send-tx  <command>",
  builder: {
    config: {
      type: "string"
    }
